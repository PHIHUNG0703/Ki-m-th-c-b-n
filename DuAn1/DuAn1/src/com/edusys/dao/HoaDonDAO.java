...
public class HoaDonDAO extends EduSysDAO<HoaDon, String> {
    ...
    final String INSERT_SQL = "INSERT INTO HOADON(MANV,MAKH,TONGTIEN) values (?,?,?)";
    final String UPDATE_SQL = "UPDATE HOADON SET MANV = ?, MAKH =?, TONGTIEN = ?  WHERE MAHD = ?";
    final String DELETE_SQL = "DELETE FROM HOADON WHERE MAHD = ?";
    final String SELECT_ALL_SQL = "SELECT A.MAHD ,A.MANV, C.TENNV, A.MAKH, D.TENKH, NGAYBAN,A.TONGTIEN \n"
            + "FROM HOADON A \n"
            + "INNER JOIN NHANVIEN C ON A.MANV = C.MANV \n"
            + "INNER JOIN KHACHHANG D ON A.MAKH = D.MAKH\n"
            + "ORDER BY NGAYBAN DESC";
    final String SELECT_BY_ID_SQL = "SELECT A.MAHD ,A.MANV, C.TENNV, A.MAKH, D.TENKH, NGAYBAN,A.TONGTIEN\n"
            + "FROM HOADON A \n"
            + "INNER JOIN NHANVIEN C ON A.MANV = C.MANV\n"
            + "INNER JOIN KHACHHANG D ON A.MAKH = D.MAKH\n"
            + "WHERE MAHD LIKE ?";
    final String SELECT_BY_KEY_WORD_SQL = "SELECT A.MAHD ,A.MANV, C.TENNV, A.MAKH, D.TENKH, NGAYBAN,A.TONGTIEN\n"
            + "FROM HOADON A \n"
            + "INNER JOIN NHANVIEN C ON A.MANV = C.MANV\n"
            + "INNER JOIN KHACHHANG D ON A.MAKH = D.MAKH\n"
            + "WHERE MAHD like ?";
    ...
    final String SHOW_ALL = "SELECT A.MAHD ,A.MANV, C.TENNV, A.MAKH, D.TENKH ,A.TONGTIEN\n"
            + "FROM HOADON A \n"
            + "INNER JOIN NHANVIEN C ON A.MANV = C.MANV\n"
            + "INNER JOIN KHACHHANG D ON A.MAKH = D.MAKH";
